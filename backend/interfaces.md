...
- `bro_name`：必填，用于指定要处理的 bro。
- `type`：必填，目前仅支持 `"txt"`（纯文本）。
- `payload`：必填，原始聊天记录内容。
- `options`：可选，解析选项（如是否仅提取文本、指定我的名称等）。

响应数据（`data`）为空，注意虽然这里没有相应，但是系统会自己把解析后的聊天记录存储起来，后续用于蒸馏。
存储在目录/chatlogstr/。
存储的文件名{bro\_name}.txt，内容就为解析后的聊天记录字符串。
